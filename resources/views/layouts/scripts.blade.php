...
<script type="text/javascript">
   ...
   //      }
                ...
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                    	...
                    	contenido+='<tr>'+
                                ...
                                '<td><a href="#" class="btn btn-success btn-sm" onclick="PagarMesEfectivo(1,'+data[i].id+','+anio+','+mes+','+i+')">Ingresar Pago en Efectivo</a></td>'+
                                '<td><span id="total_multas'+i+'"></span></td>'+
                                '<td><ul id="mis_multas'+i+'"></ul></td>'+
                                '<td><ul id="mis_status_mr'+i+'"></ul></td>'+
                                '<td><a href="#" class="btn btn-success btn-sm" onclick="PagarMesEfectivo(2,'+data[i].id+','+anio+','+mes+','+i+')">Ingresar Pago en Efectivo</a></th>'+
                            '</tr>';
                    }
    ...
    function PagarMesEfectivo(opcion,id_residente,anio,mes) {
    	$('#CargandoPagoEfectivo').show();
    	$('#PagarMesEfectivo').modal('show');
    	$('#contenidoPagoEfectivo').empty();

    	if (opcion == 1) {
    		$('#TituloPagoEfectivo').html('- Inmueble');
    		$.get('arriendos/'+id_residente+'/'+anio+'/'+mes+'/mostrar_inmuebles',function(data){
        		if(data.length>0){
        			for (var i = 0; i < data.length; i++) {
        				$('#id_mis_inmuebles'+numero).append('<li>'+data[i].id_inmueble+'</li>');
    				...
    				$('#contenidoPagoEfectivo').append('<center><h1>¡No hay nada que pagar!</h1></center>');
        		}
        	});
    	}else{
    		$('#TituloPagoEfectivo').html('- Multas/Recargas');
    		$('#contenidoPagoEfectivo').append();
    	}
    }
</script>
<!-- Plugin js-->
